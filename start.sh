...
if [ "$APM_START" -ne "1" ]; then
  echo "APM_START is set to something different from 1, not starting..."
else
  # override APM_HEAP_SIZE variable if set
  if [ ! -z "$APM_HEAP_SIZE" ]; then
    awk -v LINE="-Xmx$APM_HEAP_SIZE" '{ sub(/^.Xmx.*/, LINE); print; }' ${APM_PATH_SETTINGS}/jvm.options \
        > ${APM_PATH_SETTINGS}/jvm.options.new && mv ${APM_PATH_SETTINGS}/jvm.options.new ${APM_PATH_SETTINGS}/jvm.options
    awk -v LINE="-Xms$APM_HEAP_SIZE" '{ sub(/^.Xms.*/, LINE); print; }' ${APM_PATH_SETTINGS}/jvm.options \
        > ${APM_PATH_SETTINGS}/jvm.options.new && mv ${APM_PATH_SETTINGS}/jvm.options.new ${APM_PATH_SETTINGS}/jvm.options
  fi

  if [ ! -z "$APM_HEAP_DISABLE" ]; then
    awk -v LINE="#-XX:+HeapDumpOnOutOfMemoryError" '{ sub(/^-XX:\+HeapDumpOnOutOfMemoryError.*/, LINE); print; }' ${APM_PATH_SETTINGS}/jvm.options \
        > ${APM_PATH_SETTINGS}/jvm.options.new && mv ${APM_PATH_SETTINGS}/jvm.options.new ${APM_PATH_SETTINGS}/jvm.options
  fi

  # override APM_OPTS variable if set
  if [ ! -z "$APM_OPTS" ]; then
    awk -v LINE="APM_OPTS=\"$APM_OPTS\"" '{ sub(/^APM_OPTS=.*/, LINE); print; }' /etc/init.d/logstash \
        > /etc/init.d/logstash.new && mv /etc/init.d/logstash.new /etc/init.d/logstash && chmod +x /etc/init.d/logstash
  fi
...
if [[ "$ELASTICSEARCH_START" -ne "1" ]] && [ "$LOGSTASH_START" -ne "1" ] \
  && [[ "$KIBANA_START" -ne "1" ]] && [ "$APM_START" -ne "1" ]; then
